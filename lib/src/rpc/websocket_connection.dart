...
import 'package:solana_jsonrpc/types.dart';
...
mixin WebsocketConnection {
  ...
  void _close(final SubscriptionId subscriptionId) => _notifiers.remove(subscriptionId)?.close();
  ...
  Future<WebsocketSubscription<T>> subscribe<T>(
    ...
    final JsonRpcSubscribeResponse response = await websocketClient.send(
      method.request(commitment), method.response, config: websocketClientConfig,
    );
  ...
  Future<bool> _unsubscribe(final String method, final SubscriptionId subscriptionId) async {
    _close(subscriptionId);
    final RegExp suffixRegexp = RegExp(r'(Subscribe|Unsubscribe|Notification)$');
    assert(suffixRegexp.hasMatch(method));
    final String unsubscribeMethod = method.replaceFirst(suffixRegexp, 'Unsubscribe');
